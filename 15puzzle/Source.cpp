...
{
public:
	Node* parent; // родитель текущей €чейки
	vector<size_t> tag; // текущее состо€ние п€тнашек
	...
	Node(vector<size_t> vectTag, int empty = -1, int cur = -1, Node* parent = nullptr)
	{
		...
		// если мы в начальном положении п€тнашек
		if (cur == -1)
		{
			empty = indOf(0) % 4;
			cur = indOf(0) / 4;
		...
		if (curPos > 0)
		{
			if (this->parent != nullptr)
			{
				if (this == startNode || this->curPos != this->parent->curPos + 1)
				{
					...
					// передвигаем пустую €чейку
					nextTag[emptyPos + (curPos - 1) * 4] = 0;
					// добавл€ем новый возможный шаг
...
// ћанхэттенское рассто€ние - рассто€ние от фишки до ее правильного места
...
int main()
{
	...
	//string start = "123456789AFB EDC"; // нет решени€
	//string start = "F2345678A BE91CD"; // нет решени€
	...
	/*	Node* res = AStar(firstPosTag);
		...
		printf(" оличество шагов до решени€: %d, врем€ выполнени€: %d мсек\n", stepsCount - 1, endTime); */
